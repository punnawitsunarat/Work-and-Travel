...
print(f'Processing {ws_src.max_row - 1} public jobs with realistic hours and shift dynamics...')
...
def parse_realistic_hours_and_shifts(emp_name, state_name, state_code, pos_name, agency):
    e_low = emp_name.lower()
    s_low = state_name.lower()
    p_low = pos_name.lower()
    
    # 1. National Parks & Concessionaires (Yellowstone, Grand Teton, Glacier, Yosemite, Denali, Grand Canyon, Crater Lake, Zion)
    if any(k in e_low for k in ['xanterra', 'princess', 'gtlc', 'grand teton lodge', 'glacier national', 'yosemite', 'crater lake', 'zion national', 'grand canyon', 'denali', 'mckinley', 'signal mountain', 'pursuit', 'under canvas']):
        shifts = "ปกติ (พ.ค.): 07:30 – 16:00 (8.5 ชม.)\nพีค (มิ.ย.-ส.ค.): 07:00 – 17:30 (10.5 ชม. หมุนเวียน)"
        hours = "เฉลี่ย: 40–44 ชม./wk\nพีค ก.ค.-ส.ค.: 48–55 ชม./wk\n(งานสม่ำเสมอ ตารางแน่นอน ไม่ตัดกะ อาหาร EDR ครบ 3 มื้อ)"
        return shifts, hours

    # 2. Mega Theme Parks & Waterparks (Cedar Point, Kings Island, Kalahari, Dollywood, Six Flags, Carowinds, Busch Gardens, Silver Dollar, Mt. Olympus, Noah's Ark, Wilderness, Morey, Palace Playland, Lagoon)
    elif any(k in e_low for k in ['six flags', 'cedar point', 'dollywood', 'carowinds', 'kings island', 'busch gardens', 'silver dollar', 'wonderworks', 'kalahari', 'olympus', 'noah', 'wilderness', 'waterpark', 'amusement', 'lagoon', 'morey', 'palace playland', 'chula vista', 'white water']):
        shifts = "ปกติ (พ.ค.): 08:30 – 16:30 (8 ชม.)\nพีค (มิ.ย.-ส.ค.): 07:30 – 18:30 (11 ชม. ควงกะ/OT)"
        hours = "เฉลี่ย: 42–48 ชม./wk\nพีค ก.ค.-กลาง ส.ค.: 50–60 ชม./wk\n(ปลาย ส.ค. ฝรั่งเปิดเทอม ขาดคนหนัก OT พุ่ง 55–65 ชม.)"
        return shifts, hours

    # 3. Beach Towns & Island Resorts (Put-in-Bay, Mackinac, Outer Banks, Ocean City, Myrtle Beach, Virginia Beach, Wildwood, Cape Cod, Rehoboth, Destin, Panama City Beach, San Juan Island)
    elif any(k in e_low for k in ['put-in-bay', 'mackinac', 'outer banks', 'ocean city', 'myrtle beach', 'virginia beach', 'wildwood', 'cape cod', 'rehoboth', 'destin', 'panama city', 'roche harbor', 'san juan', 'block island', 'harborwalk']):
        shifts = "ปกติ (พ.ค.): 09:00 – 17:00 (8 ชม.)\nพีค (มิ.ย.-ส.ค.): 08:00 – 18:30 (10.5 ชม. ควงกะ)"
        hours = "เฉลี่ย: 42–46 ชม./wk\nพีค มิ.ย.-ส.ค.: 50–58 ชม./wk\n(เมืองตากอากาศคึกคัก ปลาย ส.ค. ควบงานสองง่าย รายได้พุ่ง)"
        return shifts, hours

    # 4. Busy Seafood, Steakhouses, Diners, Pizzerias, and Bar & Grills
    elif any(k in e_low for k in ['seafood', 'crab', 'steak', 'grill', 'bistro', 'diner', 'restaurant', 'cafe', 'shady gators', 'moosejaw', 'buffalo phil', 'prospector', 'mountain high', 'paula deen', 'bayou bill', 'fudpucker', 'mudhen', 'tavern', 'saloon', 'pizzeria', 'pizza pub', 'clam', 'lobster']):
        shifts = "ปกติ (พ.ค.): 11:30 – 21:00 (มีพักเบรก)\nพีค (มิ.ย.-ส.ค.): 10:30 – 23:00 (ควงกะรอบค่ำ ทิปแน่น)"
        hours = "เฉลี่ย: 40–45 ชม./wk\nพีค มิ.ย.-ส.ค.: 48–56 ชม./wk\n(กะค่ำลูกค้าแน่น ทิปสดรายวัน $50–$100/คืน ควงกะได้เยอะ)"
        return shifts, hours
    ...
    # 5. Thai Restaurants
        ...
        hours = "เฉลี่ย: 42–48 ชม./wk\nพีค มิ.ย.-ส.ค.: 50–58 ชม./wk\n(ควงกะเที่ยง-เย็น อาหารไทยฟรี 3 มื้อ บรรยากาศครอบครัว)"
        return shifts, hours

    # 6. Hotels, Luxury Resorts & Housekeeping
    elif any(k in e_low for k in ['hotel', 'inn', 'resort', 'suites', 'motel', 'lodge', 'marriott', 'hilton', 'hyatt', 'sheraton', 'westgate', 'fairfield', 'courtyard', 'four seasons', 'stanley', 'omni', 'cliff house', 'tenaya', 'sagamore', 'kiawah', 'trapp', 'cherry tree', 'eureka inn', 'brighton']):
        shifts = "ปกติ (จ.-พฤ.): 08:30 – 15:30 (7 ชม.)\nพีค (ศ.-อา.): 08:00 – 17:00 (9 ชม. วันแขกเช็คอิน)"
        hours = "เฉลี่ย: 36–42 ชม./wk\nพีค ศุกร์-อาทิตย์: 44–50 ชม./wk\n(เลิกงาน 15:30–16:30 น. เหมาะมากสำหรับต่อกะค่ำงานที่ 2)"
        return shifts, hours

    # 7. Fast Food & Casual Chains (McDonald's, Wendy's, Culver's, Five Guys, Domino's, Subway, Auntie Anne's, Dairy Queen)
        ...
        shifts = "กะเช้า: 07:00 – 15:30 (8.5 ชม.)\nกะบ่าย/ดึก: 15:30 – 00:00 (8.5 ชม.)"
        hours = "เฉลี่ย: 38–42 ชม./wk\nพีค มิ.ย.-ส.ค.: 44–50 ชม./wk\n(เวลาเข้างานแน่นอน ยืดหยุ่นสูง ปรับตารางควบงานสองง่าย)"
        return shifts, hours

    # 8. Ice Cream, Candy, Bakery & Retail
    elif any(k in e_low for k in ['creamery', 'ice cream', 'kilwins', 'fudgery', 'candy', 'kohr', 'sweets', 'chocolate', 'bakery', 'donut', 'baskin', 'gift shop', 'store', 'market', 'outlets', 'supermarket', 'mart', 'retail', 'bargain world']):
        shifts = "กะกลางวัน: 10:00 – 17:00 (7 ชม.)\nกะค่ำพีค: 16:30 – 23:30 (7 ชม. ลูกค้าแน่น)"
        hours = "เฉลี่ย: 38–44 ชม./wk\nพีค มิ.ย.-ส.ค.: 46–54 ชม./wk\n(ช่วงค่ำคนต่อคิวแน่น กะงานสนุกสนาน ไม่เหนื่อยเกินไป)"
        return shifts, hours

    # 9. General Default
    else:
        shifts = "ปกติ (พ.ค.): 08:30 – 16:30 (8 ชม.)\nพีค (มิ.ย.-ส.ค.): 08:00 – 17:30 (9.5 ชม.)"
        hours = "เฉลี่ย: 38–44 ชม./wk\nพีค มิ.ย.-ส.ค.: 46–52 ชม./wk\n(ช่วงพีคนักท่องเที่ยวหนาแน่น OT เปิดกว้าง)"
        return shifts, hours
...
for r in range(2, ws_src.max_row + 1):
    ...
    shifts_clean, hours_clean = parse_realistic_hours_and_shifts(emp_display, state_name, state_code, pos_display, agency)
...
col_widths_s1 = [12, 18, 40, 22, 35, 36, 28, 14, 45, 18, 60]
col_widths_s2 = [12, 18, 40, 22, 35, 36, 28, 14, 45, 18, 60, 42, 42, 42]
...
print('Build complete: Successfully upgraded Realistic Work Hours and Shifts for all 1,167 jobs!')
