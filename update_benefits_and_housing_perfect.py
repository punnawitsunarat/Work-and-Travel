...
print(f'Processing {ws_src.max_row - 1} public jobs with refined housing and benefits...')
...
def generate_benefits_and_housing(emp_name, state_name, state_code, pos_name, raw_housing, raw_desc):
    ...
    # 3-Meal / EDR Full Food Included detection
    ...
    # Clean weekly housing cost calculation
    hsg_val_weekly = 110
    ...
    if m:
        ...
        if raw_num == 0:
            is_free_hsg = True
        elif raw_num < 30: # per day rate
            hsg_val_weekly = int(raw_num * 7)
        elif raw_num > 1200: # full summer season package (divide by 16)
            hsg_val_weekly = int(raw_num / 16)
        elif raw_num > 350: # monthly rate (divide by 4.33)
    ...
    # Generate Benefits & Key Highlights
    perks = []
    highlights = []
...
col_widths_s1 = [12, 18, 40, 22, 35, 32, 28, 14, 18, 20, 60]
col_widths_s2 = [12, 18, 40, 22, 35, 32, 28, 14, 18, 20, 60, 42, 42, 42]
...
print('Build complete: Successfully upgraded Benefits & Highlights and Housing for all 1,167 jobs!')
